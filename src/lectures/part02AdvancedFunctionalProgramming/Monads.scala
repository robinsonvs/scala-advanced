...
object Monads extends App {
  ...
  /*
    EXERCISES:
    1) implement a Lazy[T] monad = computation wich will only be executed when it's needed
      unit/apply
      flatMap
  ...
  /*
    Monad[T] { //List
      ...
      List(List(1,2), List(3,4)).flatten = List(List(1,2), (3,4)).flatMap(x => x) = List(1,2,3,4)
   */
